...
public class TranslateTest {
	...
	public void testGetLanguagesISOCodes() {
		...
		when(store.getStringArrayOption(Languages.LANG_STORAGE_KEY, null)).thenReturn(test);
		...
		langs.add("ro");
		langs.add("es");
		String []testResult=Languages.getPrioritizedLanguageISOCodes(store, langs);
	...
	public void testIncreasePriority() {
		...
		WSOptionsStorageAdapter store = new WSOptionsStorageAdapter() {
			...
			public void setStringArrayOption(String key, String[] values) {
				if (Languages.LANG_STORAGE_KEY.equals(key)) {
			...
			public String[] getStringArrayOption(String key, String[] defaultValues) {
				// initial values.
				if (Languages.LANG_STORAGE_KEY.contentEquals(key)) {
